...
**Learn more about how Ant Group built a Cloud Workload Protection Platform (CWPP) using Kata Containers and eBPF in their most recent [whitepaper](/collateral/kata-containers-ant-group-cwpp-ebpf_whitepaper.pdf), which is also available in the original [Chinese language](/collateral/kata-containers-ant-group-cwpp-ebpf_whitepaper_chinese.pdf).**
...
With confidential computing, customers have to have absolute trust that data is not being exposed. In environments where governments and governing bodies like the PCI Security Standards Council and Cloud Security Alliance are implementing stricter regulations—particularly around sensitive data in healthcare, finance, government, and other regulated industries—it's critical to ensure that data remains protected even from privileged system software like the hypervisor. Encryption plays a key role so the confidential or container based workloads can be coupled with the AMD Secure Encrypted Virtualization - Secure Nested Paging (SEV-SNP) family of technologies. Following our customers' needs, we are working upstream with the Kata Containers community to focus on the more advanced Secure Nested Paging (SNP) technique, which builds on SEV by adding strong memory integrity protection to prevent malicious hypervisor-based attacks. 

AMD's involvement in Kata Containers aligns with its commitment to open-source technologies, allowing for better integration and performance of its hardware in containerized applications. Teams at AMD are investing in the development of Kata Containers and actively working on enhancing the security and stability aspects of container technologies. They are working with the community and teams at IBM to further explore needs and opportunities for confidential computing technology. These contributions reflect AMD's commitment to advancing confidential computing while ensuring that its hardware is effectively leveraged for performance and security.
